...
{
    [CallbackRoute($"{AdminWarrantyRequestsRoutes.Filter}:{{status}}")]
    public class RepairFilterHandler
    {
        ...
        public async Task HandleAsync(ICallbackQuery query, string status, CancellationToken ct)
        {
            ...
            userState.Data["repairFilter"] = filter;
            userState.Data["repairPage"] = 1;
            _userStateService.SetState(userId, userState);
            await ShowRepairsPage(query.Message.ChatId.Value, query.Message, filter, 1, ct);
        }
        private async Task ShowRepairsPage(long chatId, IMessage message, string? filter, int page, CancellationToken ct)
        {
            var pagedRequest = new PagedRequest { PageNumber = page, PageSize = 5 };
            var repairs = await _mediator.Send(new GetRepairRequestsFilteredPagedQuery 
                { StatusFilter = filter, PagedRequest = pagedRequest }, ct);
            ...
            var paginationButtons = _paginationControlsBuilder
                .CreatePaginationButtons(repairs, $"{AdminWarrantyRequestsRoutes.Page}:{{0}}");
